...
const STATUS = {
  ...
  served: {
    cls: 'text-muted         bg-surface-deep     border-muted/60',
    labelKey: 'group.served',
  },
  cancelled: {
    cls: 'text-muted         bg-surface-deep     border-muted/60',
    labelKey: 'group.cancelled',
  },
...
export function OrderStatusBadge({ status }: { status: OrderItemStatus }) {
  ...
  return (
    <span
      className={`text-caption font-medium px-1.75 py-0.5 rounded-full border whitespace-nowrap ${state.cls}`}
    >
      {t(state.labelKey)}
    </span>
  )
