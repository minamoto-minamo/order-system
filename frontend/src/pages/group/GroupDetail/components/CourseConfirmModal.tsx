              ...
              <div className="bg-info-bg border border-info-border rounded-lg px-3.5 py-2 flex flex-wrap gap-1">
                {plan.menuItemIds.map((mid) => {
                  ...
                  return (
                    <span
                      key={mid}
                      className="text-label text-info bg-white border border-info-border px-2 py-0.5 rounded-full"
                    >
                      {name}
                    </span>
                  )
          ...
          <div className="bg-surface border border-divider rounded-lg px-3.5 py-2.5">
            {course.foodItems.map((fi) => {
              ...
              return (
                <div
                  key={fi.menuItemId}
                  className={`flex justify-between py-1.25 ${course.foodItems[course.foodItems.length - 1]?.menuItemId !== fi.menuItemId ? 'border-b border-surface-deep' : ''}`}
                >
