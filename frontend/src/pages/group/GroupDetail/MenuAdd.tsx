...
export function MenuAdd({ menus, categories, subCategories, onAdd }: {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
            ...
            return (
              <div key={item.id} className="px-5 py-3 border-b border-surface flex items-center gap-3 bg-white">
                ...
                <div className="flex items-center gap-2">
                  {qty > 0 && (
                    <>
                      <button className="action-btn w-7.5 h-7.5 rounded-full border border-line bg-white text-base text-dim flex items-center justify-center" onClick={() => setQty(item.id, qty - 1)}>−</button>
                      <span className="text-sub font-medium text-ink min-w-5 text-center">{qty}</span>
                    </>
                  )}
                  <button
                    className={`action-btn w-7.5 h-7.5 rounded-full border text-base flex items-center justify-center ${qty > 0 ? 'border-ink bg-ink text-white' : 'border-line bg-white text-dim'}`}
      ...
      {totalCount > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 px-5 pt-3 pb-6 bg-white border-t border-divider animate-[slideUp_0.2s_ease_both]">
          <Button
