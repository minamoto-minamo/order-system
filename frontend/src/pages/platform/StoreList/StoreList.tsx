...
export default function StoreList() {
  ...
  } = useForm<StoreForm>(EMPTY_FORM)
  const showToast = useToastStore((state) => state.showToast)

  useEffect(() => {
    api
      .get<Store[]>(EP.platformStores)
      .then((list) => {
        setLoadError(false)
        setStores(list)
      })
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const updated = await api.put<Store>(EP.platformStore(toggleTarget.id), {
  ...
  return (
    <div className="app-shell h-dvh bg-white flex flex-col overflow-hidden">
      ...
      <BottomSheetModal
        ...
        title={
          toggleTarget
            ? t(toggleTarget.isActive ? 'platform.deactivateConfirm' : 'platform.activateConfirm', {
        ...
        primaryAction={{
          label: toggleTarget?.isActive ? t('platform.deactivate') : t('platform.activate'),
