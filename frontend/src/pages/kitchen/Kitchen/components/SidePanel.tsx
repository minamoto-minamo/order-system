      ...
      <div className="side-panel w-75 bg-white border-l border-divider flex flex-col h-full animate-[slideIn_0.2s_ease_both]">
        ...
        <div className="flex-1 px-5 py-4 overflow-y-auto">
          {readyItems.length > 0 && (
            <div className="mb-4">
              ...
              {readyItems.map((o) => (
                <div
                  key={o.id}
                  className="px-2.5 py-2 mb-2 bg-amber-bg border border-amber-border rounded-lg flex items-center gap-2"
                >
                  <span className="flex-1 text-note text-secondary">{o.item}</span>
                  <span className="text-xs text-muted">×{o.qty}</span>
                  <button
                    type="button"
                    className="complete-btn bg-amber-bg border border-amber-border rounded-[5px] px-2 py-3 text-caption leading-4 text-amber-fg"
          ...
          {pendingItems.length > 0 && (
            <div>
              <div className="text-label text-muted font-medium mb-2 tracking-[0.08em]">
                {t('kitchen.pending')}
              </div>
              {pendingItems.map((o) => {
                ...
                return (
                  <div
                    key={o.id}
                    className="py-2.5 border-b border-surface flex items-center gap-2"
                  >
