  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3 p-4 animate-[fadeIn_0.25s_ease_both]">
      {groups.map((g) => {
        ...
        const openGroup = () => onCardClick(g.id)
        return (
          <div
            key={g.id}
            role="button"
            ...
            <div className="px-3.5 py-2.25 bg-brand flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-white/15 px-2 py-px text-label text-white">
                    {t('kitchen.pendingCount', { count: groupOrders.length })}
                  </span>
                </div>
            ...
            <div className="py-2">
              {groupOrders.length === 0 ? (
                ...
                groupOrders.map((o) => (
                  ...
                  <div
                    key={o.id}
                    className="flex items-center px-3.5 py-1.75 gap-2 border-b border-surface last:border-b-0"
                  >
                    <span className="min-w-0 flex-1 truncate text-caption font-medium text-ink">
                      {o.item}
                    </span>
                    <span className="shrink-0 rounded-full bg-surface-deep px-2 py-0.5 text-caption text-secondary">
                      ×{o.qty}
                    </span>
                    <button
                      type="button"
                      className="complete-btn shrink-0 bg-white border border-line rounded-md px-2.5 py-3 text-caption leading-4 text-dim"
                      onClick={(e) => {
                        e.stopPropagation()
                        onComplete(o.id)
                      }}
