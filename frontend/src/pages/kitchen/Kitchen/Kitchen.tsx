import type { Category, Group, MenuItem, OrderItem, Seat, SubCategory } from '@order-system/shared'
...
export default function Kitchen() {
  ...
  const displayOrders = useMemo(
    () =>
      orders.map((o) =>
        buildDisplay(o, menus, groups, seats, (id) => t('common.unknownGroup', { id })),
      ),
    ...
    <>
      <AppHeader
        ...
        title={
          <div className="flex min-w-0 items-center gap-1.5 flex-wrap">
            <span className="text-sub font-medium text-ink whitespace-nowrap">
              {t('kitchen.title')}
            </span>
            {pendingCount > 0 && (
      ...
      <ActionBar
        right={
          <ToggleButtonGroup
            options={[
              { key: 'ticket', label: t('kitchen.ticketView') },
              { key: 'card', label: t('kitchen.groupView') },
            ]}
            value={view}
            onChange={setView}
          />
