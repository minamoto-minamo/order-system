...
const S = {
  ...
  bill: {
    bg: 'var(--color-bill-bg)',
    border: 'var(--color-bill-border)',
    text: 'var(--color-ink)',
  },
  selected: {
...
export function FloorSeat({ seat, status, group, readyCount, isSelected, onTap, G }: Props) {
  ...
  return (
    ...
    >
      <span className="text-caption font-medium" style={{ color: st.text }}>
        {seat.label}
      </span>
      {status === 'bill' && (
      ...
      {status === 'occupied' &&
        group &&
        (readyCount > 0 ? (
          <span className="inline-flex items-center gap-0.5 text-nano font-medium text-amber-fg">
