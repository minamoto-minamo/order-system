...
export default function Hall() {
  ...
  useSocketListeners({
    ...
    [SE.seatLayoutUpdated]: (layout: SeatLayoutResponse) => {
      ...
      setSeats(layout.seats.map((s) => ({ ...s, x: s.x * gs, y: s.y * gs })))
      setSeatTables(
        layout.tables.map((t) => ({ ...t, x: t.x * gs, y: t.y * gs, w: t.w * gs, h: t.h * gs })),
      )
    },
    [SE.seatCreated]: (s: Seat) =>
      setSeats((prev) => [...prev, { ...s, x: s.x * gridSize, y: s.y * gridSize }]),
    [SE.seatUpdated]: (s: Seat) =>
      setSeats((prev) =>
        prev.map((x) => (x.id === s.id ? { ...s, x: s.x * gridSize, y: s.y * gridSize } : x)),
      ),
  ...
  const readyCountByGroup = useMemo(() => {
    ...
    readyOrders.forEach((o) => {
      counts[o.groupId] = (counts[o.groupId] ?? 0) + 1
    })
    return counts
  }, [readyOrders])
  ...
  const selectedEmptySeats = selectedIds.filter(
    (id) => !groups.some((g) => g.seatIds.includes(id) && isGroupActive(g)),
  )
  ...
  return (
    <>
      ...
      <div
        className="flex-1 overflow-auto p-4 animate-[fadeIn_0.3s_ease_both]"
      ...
      >
        ...
        >
          {tables.map((table) => {
            ...
            const ids = tableSeats.map((s) => s.id)
            const isSelected =
              !hasOccupied && ids.length > 0 && ids.every((id) => selectedIds.includes(id))
            return (
              <FloorTable
                key={table.id}
                table={table}
          ...
          {seats.map((seat) => {
            ...
            const group =
              groups.find((g) => g.seatIds.includes(seat.id) && isGroupActive(g)) ?? null
            return (
              <FloorSeat
                key={seat.id}
                seat={seat}
