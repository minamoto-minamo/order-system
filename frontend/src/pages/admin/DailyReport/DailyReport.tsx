...
export default function DailyReport() {
  ...
  useEffect(() => {
    api
      .get<SessionInfo[]>(`${EP.sessions}?status=closed`)
      .then((ss) => {
        setLoadError(false)
        setSessions(ss)
  ...
  useEffect(() => {
    ...
    api
      .get<ReportData>(EP.sessionReport(selectedId))
      .then((r) => {
        setLoadError(false)
        setData(r)
        ...
        cats.forEach((c, i) => {
          catMap[c] = PALETTE[i % PALETTE.length]
        })
        const subMap: Record<string, string> = {}
        // cats.length 分オフセットしてカテゴリとサブカテゴリで色が隣接しないよう分散
        subs.forEach((s, i) => {
          subMap[s] = PALETTE[(i + cats.length) % PALETTE.length]
        })
          ...
          <>
            ...
            <div className="grid gap-2.5 mb-4 animate-[fadeIn_0.25s_ease_both] grid-cols-[repeat(auto-fill,minmax(120px,1fr))]">
              {[
                ...
                {
                  label: t('report.groupAvg'),
                  value: `¥${groupAvg.toLocaleString()}`,
                  sub: t('report.perGroup'),
                },
                { label: t('report.guests'), value: `${data.guests}名`, sub: null },
                {
                  label: t('report.guestAvg'),
                  value: `¥${guestAvg.toLocaleString()}`,
                  sub: t('report.perGuest'),
                },
              ].map((card) => (
