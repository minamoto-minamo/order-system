...
export function HourlyChart({ hourly, catNames, catColorMap }: {
  hourly: HourlyEntry[];
  catNames: string[];
          ...
          return (
            <div key={h.hour} className="flex flex-col items-center gap-1 flex-1">
              ...
              <div className="flex flex-col-reverse w-full max-w-8">
                {segments.map((seg, i) => (
                  <div key={seg.cat} className="w-full" style={{
                    height: seg.height,
      ...
      <div className="flex gap-3.5 mt-1 flex-wrap">
        {catNames.map(cat => (
          <div key={cat} className="flex items-center gap-1.25">
            <div className="w-2 h-2 rounded-sm" style={{ background: catColorMap[cat] ?? 'var(--color-line)' }} />
