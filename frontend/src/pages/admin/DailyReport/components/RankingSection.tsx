      ...
      <div className="px-4 py-3 border-b border-surface-deep flex items-center justify-between flex-wrap gap-2">
        ...
        <ToggleButtonGroup
          options={[
            ...
            { key: 'qty', label: t('report.quantity') },
          ]}
          value={metric}
          onChange={setMetric}
        />
      ...
      {sorted.length === 0 ? (
        ...
        sorted.map((item, i) => {
          ...
          return (
            <div
              key={item.name}
              className={`rank-row px-4 py-2.75 flex items-center gap-2.5 ${i < sorted.length - 1 ? 'border-b border-surface' : ''}`}
            >
              <div
                className="w-6 h-6 rounded-md shrink-0 border flex items-center justify-center text-label font-bold"
                ...
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    <span
                      ...
                      style={{ fontWeight: item.rank <= 3 ? 500 : 400 }}
                    >
                      {item.name}
                    </span>
                    <span
                      className="text-micro px-1.25 py-px rounded-full border"
                      style={{
                        color: catColor,
