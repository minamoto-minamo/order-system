...
import type { DragState, SeatData, SelectedItem, TableData } from './components/types'
import { hitTest, snap } from './components/utils'
...
export default function SeatLayout() {
  ...
  useEffect(() => {
    api
      .get<SeatLayoutResponse>(EP.seatLayout)
      .then((data) => {
        setLoadError(false)
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent, kind: 'table' | 'seat', id: number) => {
      e.preventDefault()
      e.stopPropagation()
      setSelected({ kind, id })
      ...
      if (kind === 'seat') {
        ...
        drag.current = {
          kind,
          id,
          ox: e.clientX - rect.left - seat.x,
          oy: e.clientY - rect.top - seat.y,
        }
      } else {
    ...
    (type: string, clientX: number, clientY: number) => {
      ...
      if (type === 'table') {
        ...
        const newTable: TableData = {
          id: tableId,
          label: t('seatEditor.defaultTableLabel', { n: tables.length + 1 }),
          x,
          y,
          w: gridSize * 3,
          h: gridSize * 2,
  ...
  const handleGridSizeChange = useCallback(
    (newG: number) => {
      ...
      setSeats((prev) =>
        prev.map((s) => ({ ...s, x: snap(s.x * ratio, newG), y: snap(s.y * ratio, newG) })),
      )
      setTables((prev) =>
        prev.map((t) => ({
          ...t,
          x: snap(t.x * ratio, newG),
          y: snap(t.y * ratio, newG),
          w: snap(t.w * ratio, newG),
          h: snap(t.h * ratio, newG),
        })),
      )
      setGridSize(newG)
    },
  ...
  const paletteItems = [
    {
      type: 'table',
      icon: SYMBOL_ICONS.table,
      label: t('seatEditor.table'),
      sub: t('seatEditor.tableSub'),
    },
    {
      type: 'seat',
      icon: SYMBOL_ICONS.seat,
      label: t('seatEditor.seat'),
      sub: t('seatEditor.seatSub'),
    },
  ]
