...
export function Palette({
  ...
  return (
    <div
      className={`bg-white border-r border-divider shrink-0 flex flex-col overflow-y-auto overflow-x-hidden transition-[width] duration-200 ${sidebarOpen ? 'w-39' : 'w-10'}`}
    >
      <BaseButton
      ...
      <div className={`flex flex-col flex-1 ${sidebarOpen ? 'px-3 pb-4' : 'px-1.5 pb-3'}`}>
        ...
        {items.map((item) => (
          ...
          >
            <div
              className={`w-6.5 h-6.5 bg-surface border border-line flex items-center justify-center text-xs text-secondary shrink-0 ${item.type === 'seat' ? 'rounded-full' : 'rounded-[5px]'}`}
            >
        ...
        {sidebarOpen && (
          <div className="text-caption text-dim text-center pt-1.5 pb-3.5 leading-[1.7]">
            {t('seatEditor.dragHintLine1')}
            <br />
            {t('seatEditor.dragHintLine2')}
          </div>
        )}
        {sidebarOpen && (
          <div className="mt-3 pt-3 border-t border-divider">
            <div className="text-label text-muted tracking-widest mb-2">
              {t('seatEditor.canvasSize')}
            </div>
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  value={cols}
                  onChange={(e) =>
                    onColsChange(Math.max(colsMin, Math.min(colsMax, Number(e.target.value))))
                  }
              ...
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  value={rows}
                  onChange={(e) =>
                    onRowsChange(Math.max(rowsMin, Math.min(rowsMax, Number(e.target.value))))
                  }
                  className="input-field w-14 text-center text-note"
                />
                <span className="text-caption text-dim">{t('seatEditor.canvasUnit')}</span>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    onGridSizeChange(
                      Math.max(gridSizeMin, Math.min(gridSizeMax, Number(e.target.value))),
                    )
                  }
                  className="input-field w-14 text-center text-note"
                />
