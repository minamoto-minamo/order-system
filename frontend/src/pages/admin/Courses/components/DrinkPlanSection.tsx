...
export function DrinkPlanSection({ drinkPlans, menus, onAdd, onEdit }: {
  ...
  return (
    <section className="px-5 pt-5 pb-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="text-note text-info" onClick={onAdd}>
      ...
      {drinkPlans.length === 0 ? (
        ...
        <div key={plan.id} className="py-3 border-b border-surface flex items-start gap-3">
          ...
          <button className="text-xs text-info shrink-0 pt-0.5" onClick={() => onEdit(plan)}>
            {t('common.edit')}
          </button>
        </div>
      ))}
