  ...
  return (
    <BottomSheetModal
      ...
      primaryAction={{
        ...
        disabled: !valid,
        onClick: () =>
          valid && onSave({ name: name.trim(), price: priceNum, drinkPlanId, foodItems }),
      }}
    >
      <div className="px-6 pt-6 pb-4 border-b border-divider flex items-center justify-between">
        ...
        <BaseButton
          className="w-7 h-7 flex items-center justify-center rounded text-muted text-note"
          onClick={onClose}
      ...
      <div className="px-6 pt-4 pb-3 space-y-3.5">
        <div>
          <label htmlFor={nameId} className="text-xs text-muted block mb-1.5">
            {t('courses.courseNameLabel')}
          </label>
          <input
            id={nameId}
            value={name}
        ...
        <div>
          <label htmlFor={priceId} className="text-xs text-muted block mb-1.5">
            {t('courses.coursePriceLabel')}
          </label>
          <input
            id={priceId}
            type="number"
        ...
        <div>
          <label htmlFor={drinkPlanIdId} className="text-xs text-muted block mb-1.5">
            {t('courses.courseDrinkPlanLabel')}
          </label>
          <select
            id={drinkPlanIdId}
            value={drinkPlanId ?? ''}
        ...
        {categories.map((cat) => {
          ...
          return (
            <div key={cat.id} className="mb-3">
              <div className="text-caption text-muted mb-1.5 font-medium tracking-wide">
                {cat.name}
              </div>
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex items-center gap-3 py-2 border-b border-surface"
                  >
                    <span className={`flex-1 text-note ${qty > 0 ? 'text-ink' : 'text-muted'}`}>
                      {item.name}
                    </span>
