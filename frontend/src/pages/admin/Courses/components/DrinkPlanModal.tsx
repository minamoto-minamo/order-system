...
export function DrinkPlanModal({
  plan,
  menus,
  categories,
  subCategories,
  ...
  const toggleSub = (items: MenuItem[], allSelected: boolean) =>
    setSelected((prev) => {
      const next = new Set(prev)
                ...
                return (
                  <div key={sub.id} className="mb-2">
                    ...
                    {items.map((item) => (
                      ...
                      >
                        <span
                          className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 text-caption ${selected.has(item.id) ? 'bg-info border-info text-white' : 'border-line'}`}
                        >
