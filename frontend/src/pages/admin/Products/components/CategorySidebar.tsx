...
export function CategorySidebar({
  ...
  return (
    <div
      className={`${sidebarOpen ? 'w-40' : 'w-0'} bg-white border-r border-divider flex flex-col shrink-0 overflow-hidden transition-[width] duration-200`}
    >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <button
          type="button"
          className={`tappable px-4 py-2.75 text-note border-b border-surface-deep ${selectedSubId === null ? 'text-ink font-medium bg-surface' : 'text-dim bg-transparent'}`}
          onClick={onSelectAll}
        >
          {t('common.all')}
          <span className="text-label text-muted ml-1.5">{products.length}</span>
        </button>
          ...
          <div key={cat.id}>
            ...
            <div className="flex items-center pl-4 pr-3 py-2.25 border-b border-divider bg-surface">
              ...
              >
                <span className="text-caption text-dim">
                  <Icon
                    src={
                      expandedCats[cat.id] ? ACTION_ICONS.chevronDown : ACTION_ICONS.chevronRight
                    }
                  />
                </span>
            ...
            {expandedCats[cat.id] && (
              <>
                {cat.subs.map((sub) => (
                  ...
                  >
                    <button
                      ...
                      className={`flex-1 text-left text-xs ${selectedSubId === sub.id ? 'text-info-dark' : 'text-dim'}`}
                    >
                      {sub.label}
                ...
                <button
                  type="button"
                  className="tappable pl-7 pr-2.5 py-1.75 text-label text-muted border-b border-surface"
        ...
        <button
          type="button"
          className="tappable px-4 py-2.75 text-xs text-muted mt-1"
