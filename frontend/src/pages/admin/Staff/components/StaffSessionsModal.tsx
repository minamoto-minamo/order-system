...
export function StaffSessionsModal({
  target,
  sessions,
  loading,
  onClose,
  ...
  return (
    <div className="fixed inset-0 z-modal flex items-end justify-center bg-black/40">
      ...
      />
      <div className="z-10 bg-white rounded-t-2xl w-full max-w-120 px-6 pt-6 pb-10 max-h-[80vh] overflow-y-auto animate-[slideUp_0.2s_ease_both]">
          ...
          <div className="flex flex-col gap-3">
            {sessions.map((s) => (
              <div key={s.id} className="border border-line rounded-lg px-4 py-3">
                ...
                {s.ipAddress && <div className="mt-0.5 text-label text-muted">{s.ipAddress}</div>}
              </div>
            ))}
          </div>
        )}
