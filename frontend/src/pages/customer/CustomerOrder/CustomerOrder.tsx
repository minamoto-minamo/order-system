import type { Category, Group, MenuItem, OrderItem, SubCategory } from '@order-system/shared'
...
export default function CustomerOrder() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(
        `${errorMsg}${soldOutNames.length > 0 ? `（${soldOutNames.join('、')}）` : ''}`,
        'danger',
      )
    } finally {
      setSubmitting(false)
