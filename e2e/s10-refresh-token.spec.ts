...
test('アクセストークン失効後、有効な refresh_token があれば透過的にリフレッシュされ 200 が返る', async ({ page }) => {
...
test('複数タブ相当の同時リクエストでも誤って reuse 検知されず、どちらも成功する', async ({ page }) => {
  await loginAs(page, 'staff')
  await corruptAccessToken(page)
...
test('admin がスタッフの端末を強制ログアウトすると、その端末は以後 401 になる', async ({ browser }) => {
  ...
  const staffRow = adminPage.locator('div', { hasText: CREDS.staff.username }).filter({ has: adminPage.getByRole('button', { name: ja.staff.devices.button }) }).last()
