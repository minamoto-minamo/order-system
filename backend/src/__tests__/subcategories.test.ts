...
const mockTransaction = jest.fn<(
  callback: (tx: {
    subCategory: { findFirst: typeof mockSubCategoryFindFirst; delete: typeof mockSubCategoryDelete };
...
describe('POST /api/subcategories — categoryId storeId 検証', () => {
  ...
  it('他店舗の categoryId を指定した場合は 422 を返す', async () => {
    ...
    expect(res.json()).toMatchObject({ error: { code: 'subcategories.save.category_not_found', message: 'カテゴリが見つかりません' } })
  ...
  it('自店舗の categoryId なら作成できる', async () => {
    ...
    mockSubCategoryCreate.mockResolvedValue({ id: 10, name: 'ビール', categoryId: 2, sort: 0, storeId: STORE_ID })
    const res = await app.inject({
      method: 'POST',
      url: '/api/subcategories',
      headers: { cookie: `token=${token()}` },
...
describe('DELETE /api/subcategories/:id — 削除制御', () => {
  ...
  it('存在しない ID で削除すると 404 を返す', async () => {
    ...
    expect(res.json()).toMatchObject({ error: { code: 'subcategories.detail.not_found', message: 'サブカテゴリが見つかりません' } })
    expect(mockSubCategoryFindFirst).toHaveBeenCalledWith({ where: { id: 999, storeId: STORE_ID } })
  ...
  it('メニューが使用中なら 409 を返す', async () => {
    ...
    expect(res.json()).toMatchObject({ error: { code: 'subcategories.delete.in_use', message: '使用中のサブカテゴリは削除できません' } })
...
describe('PUT /api/subcategories/:id — categoryId storeId 検証', () => {
  ...
  it('自店舗の categoryId への更新は成功する', async () => {
    mockSubCategoryFindFirst.mockResolvedValue({ id: 10 })
    mockCategoryFindFirst.mockResolvedValue({ id: 2 })
    mockSubCategoryUpdate.mockResolvedValue({ id: 10, name: '日本酒', categoryId: 2, sort: 0, storeId: STORE_ID })
    const res = await app.inject({
      method: 'PUT',
      url: '/api/subcategories/10',
      headers: { cookie: `token=${token()}` },
