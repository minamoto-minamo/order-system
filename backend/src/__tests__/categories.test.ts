...
describe('DELETE /api/categories/:id — 削除制御', () => {
  ...
  it('存在しない ID で削除すると 404 を返す', async () => {
    ...
    expect(res.json()).toMatchObject({ error: { code: 'categories.detail.not_found', message: 'カテゴリが見つかりません' } })
  ...
  it('サブカテゴリが使用中なら 409 を返す', async () => {
    ...
    expect(res.json()).toMatchObject({ error: { code: 'categories.delete.in_use', message: '使用中のカテゴリは削除できません' } })
