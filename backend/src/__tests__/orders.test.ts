...
describe('POST /api/orders — 飲み放題プラン対象商品の0円化', () => {
  ...
  it('drinkPlan 対象商品でもテイクアウト注文なら通常単価のまま、originalPrice は注文時点単価を持つ', async () => {
    ...
    mockSettingFindUnique.mockResolvedValue({ taxRateInHouse: { toNumber: () => 10 }, taxRateTakeout: { toNumber: () => 8 } })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mockCreate = jest.fn<any>().mockResolvedValue({ id: 'item-1', groupId: GROUP_ID, menuItemId: 1, menuItemName: '生ビール', price: 600, originalPrice: 600, qty: 1, status: 'pending', isTakeout: true, taxRate: { toNumber: () => 8 }, courseId: null, orderedAt: new Date() })
    mockTx(mockCreate, { drinkPlanId: 5 })
