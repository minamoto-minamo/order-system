...
describe('Socket.io — order:complete / order:serve の group/session 状態チェック', () => {
  ...
  it('事前チェック時点では pending でも、更新直前にキャンセル等で状態が変わっていた場合（updateMany count: 0）、order:complete は明細を更新せず error も emit しない（サイレントno-op）', async () => {
    const socket = connect()
    mockOrderItemFindFirst.mockResolvedValue({
      id: 'item-1',
      status: 'pending',
  ...
  it('事前チェック時点では ready でも、更新直前にキャンセル等で状態が変わっていた場合（updateMany count: 0）、order:serve は明細を更新せず error も emit しない（サイレントno-op）', async () => {
    const socket = connect()
    mockOrderItemFindFirst.mockResolvedValue({
      id: 'item-1',
      status: 'ready',
