...
type GroupWithSeats = Prisma.GroupGetPayload<{ include: { seats: true } }>
type OrderItemRecord = OrderItem
type GroupResponse = ReturnType<typeof toGroup>
type CreateGroupTxResult = { group: GroupWithSeats } | { err: 'no_session' | 'seat_conflict' }
type ApplyCourseTxResult = {
...
type UpdateCourseQtyTxResult = {
  ...
  updatedFoodItems: OrderItemRecord[]
}
type DeleteCourseTxResult = {
  group: GroupWithSeats
  restoredItems: OrderItemRecord[]
  cancelledItems: OrderItemRecord[]
}
...
async function unapplyCourse(
  ...
  if (courseId != null) {
    // courseId は POST /orders 経由の通常満額注文にも付与されるため、courseId 一致だけでは
    // コース由来の明細と確定できない。定額課金明細（isCourseCharge:true）と、courseId 適用時に
    // CourseFoodItem から自動生成された個別食事明細（menuItemId が course.foodItems に含まれる）
    // のみを対象にする
    const course = await tx.course.findFirst({
      where: { id: courseId, storeId },
      include: { foodItems: true },
    })
    const foodMenuItemIds = (course?.foodItems ?? []).map((fi) => fi.menuItemId)
    const courseItems = await tx.orderItem.findMany({
      where: {
        ...
        courseId,
        status: { not: 'cancelled' },
        OR: [{ isCourseCharge: true }, { menuItemId: { in: foodMenuItemIds } }],
      },
    })
...
const groupsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/', async (request, reply) => {
    const { sessionId, status } = request.query as {
      sessionId?: string
  ...
  fastify.post('/', { schema: { body: createBodySchema } }, async (request, reply) => {
    ...
    const ownedSeats = await prisma.seat.findMany({
      where: { id: { in: body.seatIds }, storeId: request.storeId },
    })
    ...
    let txResult: CreateGroupTxResult
    try {
    ...
    if ('err' in txResult) {
      if (txResult.err === 'no_session')
        return sendError(
          reply,
          409,
          ErrorCodes.Groups.NoOpenSession,
          '営業中のセッションがありません',
        )
      return sendError(reply, 409, ErrorCodes.Groups.SeatConflict, '選択した席はすでに使用中です')
    }
  ...
  fastify.put('/:id', { schema: { body: updateBodySchema } }, async (request, reply) => {
    ...
    if (!existingGroup)
      return sendError(reply, 404, ErrorCodes.Groups.NotFound, 'グループが見つかりません')

    // 店舗内に他の open セッションがあるかではなく、対象グループ自身のセッションが営業中かを見る
    const session = await prisma.session.findFirst({
      where: { id: existingGroup.sessionId, storeId: request.storeId },
    })
    ...
    if (body.seatIds !== undefined) {
      const ownedSeatCount = await prisma.seat.count({
        where: { id: { in: body.seatIds }, storeId: request.storeId },
      })
    ...
    try {
      const group = await prisma.$transaction(
        async (tx) => {
          ...
          if (body.status !== undefined) {
            ...
            if (
              (from === 'closed' || from === 'bill_requested') &&
              (body.name !== undefined ||
                body.guestCount !== undefined ||
                body.seatIds !== undefined)
            ) {
              // status と同時に他フィールドが送られると、下の else 節のガードがスキップされてしまうため
              // ここで明示的に検査する
      ...
      if (e instanceof InvalidTransitionError)
        return sendError(
          reply,
          409,
          ErrorCodes.Groups.InvalidTransition,
          `${e.from} から ${e.to} への遷移は許可されていません`,
          { from: e.from, to: e.to },
        )
      ...
      if (e instanceof GroupStatusError)
        return sendError(
          reply,
          409,
          ErrorCodes.Groups.ClosedOrBillRequested,
          '会計済み・会計待ちのグループは変更できません',
        )
      if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2034') {
        return sendError(reply, 409, ErrorCodes.Groups.SeatConflict, '選択した席はすでに使用中です')
      }
  ...
  fastify.post(
    '/:id/course',
    { schema: { body: applyCourseBodySchema } },
    async (request, reply) => {
      const { id } = request.params as { id: string }
      ...
      try {
        ...
        if (e instanceof SoldOutError)
          return sendError(
            reply,
            409,
            ErrorCodes.Groups.CourseSoldOut,
            'コース内に品切れの商品が含まれています',
          )
        if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2034')
          return sendError(
            reply,
            409,
            ErrorCodes.Groups.CourseConflict,
      ...
      try {
        ...
        if (e instanceof SettingNotFoundError)
          return sendError(
            reply,
            500,
            ErrorCodes.Common.SettingNotFound,
            '店舗設定が見つかりません',
          )
        throw e
      }
      ...
      for (const item of [
        ...
        fastify.io
          .to(`store:${request.storeId}`)
          .to(`group:${id}`)
          .emit('order:updated', toOrderItem(item))
      }
      return groupResult
    },
  )
  ...
  fastify.put(
    '/:id/course',
    { schema: { body: updateCourseQtyBodySchema } },
    async (request, reply) => {
      const { id } = request.params as { id: string }
      ...
      if (group.status !== 'active')
        return sendError(
          reply,
          409,
          ErrorCodes.Groups.CourseQtyNotEditable,
          'このグループのコース人数は変更できません',
        )
      ...
      try {
        txResult = await prisma.$transaction(
          async (tx) => {
            ...
            const chargeResult = chargeItem
              ? await tx.orderItem.update({ where: { id: chargeItem.id }, data: { qty } })
              : null
            ...
            if (foodItemQtyByMenuItemId.size > 0) {
              ...
              for (const item of foodItems) {
                const perGuestQty =
                  item.menuItemId != null ? foodItemQtyByMenuItemId.get(item.menuItemId) : undefined
                if (perGuestQty == null) continue
                const updatedFoodItem = await tx.orderItem.update({
                  where: { id: item.id },
                  data: { qty: perGuestQty * qty },
                })
  ...
  fastify.delete('/:id/course', async (request, reply) => {
    ...
    if (existing.status !== 'active')
      return sendError(
        reply,
        409,
        ErrorCodes.Groups.CourseRemovalNotAllowed,
        'このグループのコースは解除できません',
      )
