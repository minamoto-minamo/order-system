...
const seatLayoutRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.put(
    '/',
    { schema: { body: putBodySchema }, preHandler: requireAdmin },
    async (request, reply) => {
      ...
      if (canvasCols < colsMin || canvasCols > colsMax) {
        return sendError(
          reply,
          400,
          ErrorCodes.SeatLayout.CanvasColsOutOfRange,
          `canvasCols は ${colsMin}〜${colsMax} の範囲で指定してください`,
          { min: colsMin, max: colsMax },
        )
      }
      if (canvasRows < rowsMin || canvasRows > rowsMax) {
        return sendError(
          reply,
          400,
          ErrorCodes.SeatLayout.CanvasRowsOutOfRange,
          `canvasRows は ${rowsMin}〜${rowsMax} の範囲で指定してください`,
          { min: rowsMin, max: rowsMax },
        )
      }
      if (gridSize < gsMin || gridSize > gsMax) {
        return sendError(
          reply,
          400,
          ErrorCodes.SeatLayout.GridSizeOutOfRange,
          `gridSize は ${gsMin}〜${gsMax} の範囲で指定してください`,
          { min: gsMin, max: gsMax },
        )
      }
      ...
      const invalidTableId = ownedSeats.find(
        (s) => s.tableId != null && s.tableId > 0 && !dbTableIds.has(s.tableId),
      )?.tableId
      if (invalidTableId != null) {
        return sendError(
          reply,
          422,
          ErrorCodes.SeatLayout.InvalidTableId,
          '無効なテーブルが含まれています',
