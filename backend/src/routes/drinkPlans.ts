...
type DeleteDrinkPlanTxResult = { ok: true } | { err: 'not_found' | 'referenced_course' | 'in_use' }
...
const drinkPlansRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/', async (request) => {
    const plans = await prisma.drinkPlan.findMany({
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      const existing = await prisma.drinkPlan.findFirst({
      ...
      if (!existing)
        return sendError(
          reply,
          404,
          ErrorCodes.DrinkPlans.NotFound,
          '飲み放題プランが見つかりません',
        )
      if (
        body.menuItemIds !== undefined &&
        !(await ownsMenuItems(request.storeId, body.menuItemIds))
      ) {
        return sendError(reply, 422, ErrorCodes.DrinkPlans.MenuNotFound, 'メニューが見つかりません')
      }
  ...
  fastify.delete('/:id', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    let txResult: DeleteDrinkPlanTxResult
    try {
      txResult = await prisma.$transaction(
        async (tx) => {
          const existing = await tx.drinkPlan.findFirst({
            where: { id: drinkPlanId, storeId: request.storeId },
          })
    ...
    if ('err' in txResult) {
      ...
      if (txResult.err === 'referenced_course')
        return sendError(
          reply,
          409,
          ErrorCodes.DrinkPlans.ReferencedCourse,
          'コースから参照されているため削除できません',
        )
      return sendError(
        reply,
        409,
